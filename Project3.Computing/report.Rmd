...
```{r read scan}
system.time(dat_scan <- scan(here("Project3.Computing", "bigdata",
 "ss13hus.csv.bz2"), what="character", quiet = TRUE))
 rm(dat_scan)
```

```{r read readLines}
system.time(dat_lines <- readLines(here("Project3.Computing", "bigdata",
"ss13hus.csv.bz2")))
rm(dat_lines)
```

```{r read read.csv}
system.time(dat_csv <- read.csv(here("Project3.Computing", "bigdata",
 "ss13hus.csv.bz2")))
 dat_csv <- data.table(dat_csv)
```
